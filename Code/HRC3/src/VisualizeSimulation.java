...
class EnvironmentPanel extends JPanel {
    ...
    private HumanRobotInterface humanRobotInterface;
    boolean first = true;
    ...
    public EnvironmentPanel(Environment env, ArrayList<Robot> robots) {
        ...
        this.env = env;
        this.robots = robots;
        this.humanRobotInterface = new HumanRobotInterface(robots, env);

        //Tell robots what their color is
        for (int i = 0; i < this.robots.size(); i++) {
            Robot rob = this.robots.get(i);
            if (rob.isPlayer) {
                rob.color = Properties.RED;
                rob.colorName = "RED";
            }
            else {
                rob.color = robotColors.get(i);
                rob.colorName = colorNames.get(i);
            }
        }
        ...
        this.timer = new Timer(timeStepSpeed, new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if(first){
                    repaint();
                    humanRobotInterface.getNames();
                    while (humanRobotInterface.getDialog()) { }
                    first = false;
                }
                updateEnvironment();
                repaint();
                if (goalConditionMet()) {
                    Utilities.writePlans();
                    timer.stop();
